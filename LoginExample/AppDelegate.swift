...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    static let notificationURL = ""
    static var deviceId = String()
    static let servevrKey = ""
    var window: UIWindow?
        ...
        if let aps = userInfo["aps"] as? NSDictionary {
            if let alert = aps["alert"] as? NSDictionary {
                if let message = alert["body"] as? NSString {
                   //Do stuff
                    speakMessage = message as String
                }
            }
        }
        //read the message sent
        var speechSynthesizer = AVSpeechSynthesizer()
        var speechUtterance: AVSpeechUtterance = AVSpeechUtterance(string: " \(speakMessage)")
...
extension AppDelegate: MessagingDelegate{
    func messaging(_ messaging: Messaging, didReceiveRegistrationToken fcmToken: String) {
      ...
      NotificationCenter.default.post(name: Notification.Name("FCMToken"), object: nil, userInfo: dataDict)
        print("device id assigned after registeration of token")
